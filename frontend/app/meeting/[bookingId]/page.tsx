    ...
    <div className="flex items-center gap-2 sm:gap-3">
      {isRecording ? (
        <button
          ...
          className="bg-red-600 hover:bg-red-700 px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-white font-black text-[8px] sm:text-[10px] uppercase tracking-widest flex items-center gap-1 sm:gap-2 transition-all shadow-lg active:scale-95"
        >
          <CircleStop className="w-3 h-3 sm:w-4 sm:h-4 animate-pulse" />
        ...
        <button
          ...
          className="bg-white/10 hover:bg-white/20 border border-white/10 px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-white font-black text-[8px] sm:text-[10px] uppercase tracking-widest flex items-center gap-1 sm:gap-2 transition-all active:scale-95"
        >
          <Circle className="w-3 h-3 sm:w-4 sm:h-4 text-red-500 fill-red-500" />
...
const InviteButton = () => {
  ...
  return (
    <button
      onClick={handleCopy}
      className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-lg ${
        copied 
          ? 'bg-green-600 text-white' 
          : 'bg-primary-600 hover:bg-primary-500 text-white'
      }`}
    >
      {copied ? <Check className="w-3 h-3 sm:w-4 sm:h-4" /> : <UserPlus className="w-3 h-3 sm:w-4 sm:h-4" />}
...
function MeetingContent() {
  ...
  if (!user && !guestMode) {
    return (
      <div className="fixed inset-0 bg-slate-950 flex items-center justify-center p-4 sm:p-6 z-[300]">
        <div className="max-w-md w-full bg-slate-900 border border-white/5 rounded-3xl sm:rounded-[2.5rem] p-8 sm:p-12 text-center shadow-2xl">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white mb-3 sm:mb-4 uppercase tracking-tighter">Private Session</h1>
          <p className="text-slate-500 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-8 sm:mb-10">Enter your name to join</p>
          <form onSubmit={handleGuestJoin} className="space-y-4">
            <input
              ...
              className="w-full px-4 sm:px-6 py-4 sm:py-5 bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl text-white font-bold placeholder:text-slate-600 outline-none focus:border-primary-500 transition-all text-sm sm:text-base"
              autoFocus
              required
            />
            <button
              type="submit"
              className="w-full py-4 sm:py-5 bg-primary-600 hover:bg-primary-500 text-white font-black rounded-xl sm:rounded-2xl text-xs uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2"
            >
  ...
  if (status === 'error') {
    return (
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-6 sm:p-8 z-[300]">
        <div className="max-w-md text-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-white mb-3 sm:mb-4 uppercase tracking-tighter">Connection Failed</h2>
          <p className="text-slate-400 text-sm mb-8 sm:mb-12 leading-relaxed">{errorMessage}</p>
          <div className="flex flex-col gap-4">
            <button 
              onClick={() => window.location.reload()} 
              className="w-full py-4 sm:py-5 bg-white text-slate-900 font-black rounded-xl sm:rounded-2xl uppercase text-xs tracking-widest active:scale-95 shadow-2xl"
            >
              Retry Connection
            </button>
            <button 
              onClick={() => router.push('/dashboard')} 
              className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.3em] hover:text-white transition-colors"
            >
              Return to Dashboard
            </button>
          </div>
      ...
      {videoClient && activeCall ? (
        <StreamVideo client={videoClient}>
          <StreamCall call={activeCall}>
            <StreamTheme className="flex flex-col w-full h-full overflow-hidden">
              ...
              <header className="shrink-0 h-14 sm:h-16 lg:h-20 px-3 sm:px-4 lg:px-8 flex items-center justify-between backdrop-blur-xl bg-slate-950/80 border-b border-white/5 z-50">
                <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
                  ...
                  <div className="hidden sm:block">
                    <h1 className="text-[10px] sm:text-xs font-black uppercase tracking-wider">
                      Professionals BD
                    </h1>
              ...
              <div className="flex-1 flex overflow-hidden relative">
                ...
                <div className={`flex-1 relative bg-black ${(showParticipants || showChat) ? 'hidden sm:block' : ''}`}>
                  <SpeakerLayout participantsBarPosition="bottom" />
                  {/* Call Controls - auto-hiding */}
                  <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-30 transition-all duration-300 ${
                    showControls ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 pointer-events-none'
                  }`}>
                    <div className="bg-slate-900/80 backdrop-blur-xl rounded-full border border-white/10">
                ...
                {showParticipants && !showChat && (
                  <div className="absolute sm:relative inset-0 sm:inset-auto w-full sm:w-80 xl:w-96 border-l border-white/5 bg-slate-950/95 backdrop-blur-3xl p-4 sm:p-6 z-40 overflow-y-auto no-scrollbar">
                ...
                {/* Chat Sidebar */}
                {showChat && (
                  <div className="absolute sm:relative inset-0 sm:inset-auto w-full sm:w-80 xl:w-96 border-l border-white/5 bg-slate-950/95 backdrop-blur-3xl p-4 sm:p-6 z-40 overflow-y-auto no-scrollbar flex flex-col">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <MessageSquare className="w-4 h-4 text-primary-500" />
                        <h3 className="text-xs font-black uppercase tracking-widest">Chat</h3>
                      </div>
                      <button
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 z-50">
          <div className="relative mb-6 sm:mb-8">
            <Loader2 className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 text-primary-500 animate-spin" />
            <Activity className="absolute inset-0 m-auto w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-primary-400 animate-pulse" />
          </div>
          <p className="text-xl sm:text-2xl font-black text-white uppercase tracking-[0.4em] mb-2 sm:mb-3">Connecting</p>
          <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest">Initializing secure session</p>
        </div>
      )}
